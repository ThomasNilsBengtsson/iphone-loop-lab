...
// Add a variable for your cooldown system
let cooldown = false;
let counter = 0;
let gameEnded = false; // add this variable to stop updates
  ...
  DeviceMotionEvent.requestPermission().then((response) => {
    if (response == "granted") {
      ...
      window.addEventListener("deviceorientation", (event) => {
        ...
        //Expected positions in the next frame
        let nextPx = px + vx * updateRate * 0.5;
        let nextPy = py + vy * updateRate * 0.5;

        //Expected position in pixels
        const nextPixelX = (nextPx / 100) * viewportWidth;
        const nextPixelY = (nextPy / 100) * viewportHeight;

        const lineDiv = document.getElementById("line");
        const linePosition = lineDiv.getBoundingClientRect();

        if (!cooldown) {
          // Check if the dot's center, plus its radius, would cross the line in the next frame
          if (
            nextPixelX - dotRadius <= linePosition.right &&
            nextPixelX + dotRadius >= linePosition.left &&
            nextPixelY - dotRadius <= linePosition.bottom &&
            nextPixelY + dotRadius >= linePosition.top
          ) {
            counter++;
            document.getElementById("counter").innerText = counter + "/3";
            cooldown = true;
            setTimeout(() => {
              cooldown = false;
            }, 1000);
            ...
            if (counter === 3) {
              gameEnded = true;
              // Wait a short amount of time before showing the alert
              setTimeout(() => {
                alert("the for loop is complete");
              }, 50);
            }
          } else {
            cooldown = false;
